...
namespace thegame
{
    class Client
    {
        public const byte LOGIN = 0x00;
        public const byte POSITION = 0x01;
        public const byte NEXTLEVEL = 0x02;
        public NetClient client;
        public NetConnection senderConnection = null;
        List<Perso> persos;

        public Client()
        {
            NetPeerConfiguration config = new NetPeerConfiguration("Underground");
            client = new NetClient(config);
            client.Start();
            var outmsg = client.CreateMessage();
            outmsg.Write(LOGIN);
            client.Connect("10.3.140.221", 14248, outmsg);
            senderConnection = client.ServerConnection;
                ...
                switch (inc.MessageType)
                {
                    case NetIncomingMessageType.Data:
                        ...
                        if (type == POSITION)
                        {
                            ...
                            {
                                if (p != null)
                                    p.positionPerso = new Vector2(inc.ReadFloat(), inc.ReadFloat());
                                //secondPlayer.IsJumping = inc.ReadBoolean();
                            }
                        }
                        else if (type == NEXTLEVEL)
                        {
                            //LoadNextLevel(sendToOther: false);
                        }
                        break;
                }
            }
